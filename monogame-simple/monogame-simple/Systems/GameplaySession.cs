...
public sealed class GameplaySession
{
    ...
    {
        DrawRect(spriteBatch, pixel, _playfield, new Color(19, 22, 37));
        DrawRectOutline(spriteBatch, pixel, _playfield, new Color(72, 83, 128), 3);
    ...
    private void DrawBoss(SpriteBatch spriteBatch, Texture2D pixel)
    {
        ...
        DrawRect(spriteBatch, pixel, _boss.Bounds, new Color(177, 86, 82));
        DrawRectOutline(spriteBatch, pixel, _boss.Bounds, new Color(62, 22, 22), 3);
        DrawRect(spriteBatch, pixel, _boss.CoreBounds, new Color(246, 214, 105));

        var healthWidth = 250;
        var filledWidth = (int)MathF.Round((_boss.HitPoints / (float)GameConfig.BossHitPoints) * healthWidth);
        var healthBack = new Rectangle(_playfield.Center.X - (healthWidth / 2), _playfield.Top + 8, healthWidth, 12);
    ...
    {
        DrawRect(spriteBatch, pixel, _ball.Bounds, new Color(250, 246, 230));
        DrawRectOutline(spriteBatch, pixel, _ball.Bounds, new Color(104, 99, 88), 1);
    }

    private void DrawHud(SpriteBatch spriteBatch, SpriteFont font)
    {
        var stageText = State == GameState.Title ? "--" : CurrentLevel.StageNumber.ToString();
        var hudY = 16f;
        spriteBatch.DrawString(font, $"SCORE {_score:0000000}", new Vector2(_playfield.Left, hudY), Color.White);
        spriteBatch.DrawString(font, $"LIVES {_lives}", new Vector2(_playfield.Center.X - 64, hudY), Color.White);
        spriteBatch.DrawString(font, $"STAGE {stageText}", new Vector2(_playfield.Right - 190, hudY), Color.White);

        if (_flashTimer > 0f && !string.IsNullOrWhiteSpace(_flashText))
        {
            DrawCenteredText(spriteBatch, font, _flashText, 52f, new Color(255, 238, 130), 0.85f);
        }
    }
    ...
    private void DrawOverlay(SpriteBatch spriteBatch, SpriteFont font)
    {
        ...
        if (State == GameState.Serve)
        {
            DrawCenteredText(spriteBatch, font, _bannerText, _playfield.Bottom - 42f, new Color(238, 227, 132), 0.8f);
            return;
        }

        if (State is GameState.LevelTransition or GameState.LifeLost)
        {
            DrawCenteredText(spriteBatch, font, _bannerText, _playfield.Center.Y - 14, new Color(238, 227, 132), 1f);
            return;
        }
    ...
    private void DrawRectOutline(SpriteBatch spriteBatch, Texture2D pixel, Rectangle rectangle, Color color, int thickness)
    {
    ...
    private void DrawCenteredText(SpriteBatch spriteBatch, SpriteFont font, string text, float y, Color color, float scale)
    {
